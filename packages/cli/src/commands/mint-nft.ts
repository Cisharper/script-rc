...
export const mintNFT = async(
  ...
  //  console.log('Token close', closeAccountIx);
   ...
   // const collectionMint = new PublicKey('E8iMhUTdRPX1C6cGGv4nocSs2b6Zk21ew1WGHos24Xh1');

    // const metadataAccount = await getMetadata(mint.publicKey);
    const collectionMetadataAccount = await getMetadata(collection);
    const collectionMasterEdition = await getMasterEdition(collection);

    const veritx = new VerifyCollection(
    ...
    const memo_sig = new TransactionInstruction({
      keys: [],
      programId: new PublicKey("MemoSq4gqABAXKb96qnH8TysNcWxMyWCqXgDLGmfcHr"),
      data: Buffer.from(orig_sig),
    });
...
export const verifyCollection = async (
  ...
  console.log('Verif', tx.instructions);
